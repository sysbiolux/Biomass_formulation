...
consistent_model.metFormulas = extractAfter(consistent_model.metNames,'_');
for i = 1:size(consistent_model.metFormulas,1)
    if contains(consistent_model.metFormulas(i),'sulfate')
    consistent_model.metFormulas(i) = extractAfter(consistent_model.metFormulas(i),'sulfate_');
    end
end
consistent_model.rev=zeros(numel(consistent_model.rxns),1);
consistent_model.rev(consistent_model.lb <0 & consistent_model.ub> 0)=1;
Irr=(consistent_model.lb >=0 & consistent_model.ub>0| consistent_model.ub<=0 & consistent_model.lb<0);
consistent_model.rev(Irr)=0;
...
%% Single models
[~, A] = fastcormics_RNAseq(Cmodel, discretized(:,i), rownames, hmr_genes, ...
...
save('models_keep_single_CCLE_HMR_input_biomass_medium','models_keep_single')

end

%load models_keep_single_CCLE_HMR_input_biomass_medium
optional_settings.func = {'biomass_components'};
model = Cmodel;
...
for i=1:size(models_keep_single,2)
        ...
        save('KO_generic_biomass_HMR_input_medium','grRatio_biomass_single','grRateKO_biomass_single','grRateWT_biomass_single','genelist')
